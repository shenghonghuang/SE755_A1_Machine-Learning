...
occupancy.drop(['Date','Team1_Ball_Possession(%)'],axis=1,inplace=True)
occupancy.describe()

#occupancy attributes
w_features=occupancy.iloc[:,np.arange(26)].copy()
#occupancy goal result
w_goals=occupancy.iloc[:,26].copy()
#occupancy match result
w_results=occupancy.iloc[:,27].copy()
...
occupancy_cleaned=pd.concat([feature_prepared,w_goals.to_frame(), w_results.to_frame()], axis=1)
